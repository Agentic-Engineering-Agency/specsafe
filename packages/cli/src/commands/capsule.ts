...
capsuleCommand
  ...
  .action(async (
    ...
    try {
      // Validate spec ID early to catch errors
      try {
        validateSpecId(spec);
      } catch (error) {
        console.error(chalk.red(`Error: ${error instanceof Error ? error.message : 'Invalid spec ID'}`));
        process.exit(1);
      }
      ...
      if (!title) {
        ...
        try {
          ...
          console.error(chalk.red(`Error: ${error instanceof Error ? error.message : 'Invalid title'}`));
          process.exit(1);
        }
      }

      // Get author with validation
      ...
      if (!author) {
        author = await input({
          ...
          validate: (value) => {
            try {
              ...
              return error instanceof Error ? error.message : 'Invalid author';
            }
          },
        });
      } else {
        try {
          ...
          console.error(chalk.red(`Error: ${error instanceof Error ? error.message : 'Invalid author'}`));
          process.exit(1);
        }
      }
      ...
      let tags: string[] = [];
      try {
        tags = validateTags(tagsRaw);
      } catch (error) {
        console.error(chalk.red(`Error: ${error instanceof Error ? error.message : 'Invalid tags'}`));
        process.exit(1);
      }
      ...
      console.error(chalk.red(`Error: ${error instanceof Error ? error.message : 'Failed to create capsule'}`));
      process.exit(1);
    }
  });
...
capsuleCommand
  ...
  .action(async (
    ...
    try {
      ...
      console.error(chalk.red(`Error: ${error instanceof Error ? error.message : 'Failed to list capsules'}`));
      process.exit(1);
    }
  });
...
capsuleCommand
  ...
  .action(async (spec: string, capsuleId: string) => {
    try {
      ...
      if (!capsuleId || typeof capsuleId !== 'string' || capsuleId.length === 0) {
        console.error(chalk.red('Error: Invalid capsule ID'));
        process.exit(1);
      }
      ...
      console.error(chalk.red(`Error: ${error instanceof Error ? error.message : 'Failed to show capsule'}`));
      process.exit(1);
    }
  });
...
capsuleCommand
  ...
  .action(async (
    ...
    try {
      ...
      console.error(chalk.red(`Error: ${error instanceof Error ? error.message : 'Failed to remove capsule'}`));
      process.exit(1);
    }
  });
    ...
    try {
      ...
      if (options.output) {
        if (typeof options.output !== 'string' || options.output.length === 0) {
          console.error(chalk.red('Error: Invalid output path'));
          process.exit(1);
        }

        // Security: Prevent path traversal in output path
        const normalized = options.output.replace(/\\/g, '/');
        if (normalized.includes('..')) {
          console.error(chalk.red('Error: Output path cannot contain path traversal sequences'));
          process.exit(1);
        }
      }
      ...
      console.error(chalk.red(`Error: ${error instanceof Error ? error.message : 'Failed to export capsules'}`));
      process.exit(1);
    }
  });
...
capsuleCommand
  ...
  .action(async (spec: string, capsuleId: string) => {
    try {
      ...
      const title = titleInput || capsule.title;

      const authorInput = await input({
        message: 'Author:',
        default: capsule.author,
      });
      const author = authorInput || capsule.author;
      ...
      let validatedAuthor: string;
      let validatedTags: string[];

      try {
        validatedTitle = validateTitle(title);
        validatedAuthor = validateAuthor(author);
        validatedTags = validateTags(tagsRaw.length > 0 ? tagsRaw : capsule.tags);
      } catch (error) {
        console.error(chalk.red(`Error: ${error instanceof Error ? error.message : 'Invalid input'}`));
        process.exit(1);
      }
      ...
      const content = contentInput || capsule.content;
      ...
      console.error(chalk.red(`Error: ${error instanceof Error ? error.message : 'Failed to edit capsule'}`));
      process.exit(1);
    }
  });
